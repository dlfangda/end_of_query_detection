...
def log(x):
    """A wrapper around math.log10, so that it returns 0 in the case the values is 0."""
    if x == 0.0:
        return 0
    else:
...
class LanguageModel(object):
    ...
    def tokens_logprob(self,tokens,order,special=None):
        """Returns the logprobs assigned by the model to each token for the specified order.
        The method skips the first order-1 tokens as initial context."""
        delta = self.order - 1
        return [log(self.ngram_prob(tokens[i - delta : i + 1],order,special)) for i in range(delta,len(tokens))]
    ...
    def entropy(self,text,order):
        ...
        The text is tokenized by this method (and the necessary <s> and </s> tags added)"""   
        tokens = self.tokenize_sentence(text,order)
        s = 0.
        delta = self.order - 1
        for i in xrange(delta, len(tokens)):
            ng = tokens[i - delta : i + 1]
            p = self.ngram_prob(ng,order) #got a partial word factor in this now
            s += (p * -log(p))
        return s
    ...
    def logprob_weighted_by_inverse_unigram_logprob(self,tokens,special=None):
        """WML in Clark et al. 2013. The logprob sum weighted by inverse of the sum of the unigram logprobs. 
        ...
        return sum(self.tokens_logprob(tokens,self.order,special)) / (-1. * test)
    ...
    def prob_distribution_over_completions(self,context):
        """will give us the prob_distribution_over_completions of possible continuations from these context tokens"""
        raise NameError('Method prob_distribution_over_completions not implemented')
    
    def entropy_continuation(self,tokens,order):
        """This method calculates the entropy of the prob_distribution_over_completions of following words given the context tokens,
        This may only be useful for n-1 context lengths as we don't propogate (though see Clark et al 13) a two-word context, though this is generalisable
        """
    ...
    def cache_result(self,ngram,mytuple):
        ...
        if len(self.cache.keys())>self.cache_limit:
            for key in self.cache.iterkeys():#removes any other element, hopefully first..
                if not key == str(ngram): del self.cache[key]; break
    
    def init_cache(self):
        """"Initializes a cache to speed access up"""
        self.cache = defaultdict(float) #just map from trigram to prob, no glueing etc involved
        self.cache_limit = 300 #keeps most common n sparse matrices
    
class NgramGraph(object):
    """DAG structure that stores the current probability of the whole sequence and the prob of its unigrams
    for fast computation of the liklihood of subsequences without recomputation.
    ...
    def __init__(self,lm,words=None):
        ...
        if words != None: #can initialise with words and will calculate their probs
            for word in words[self.delta:]:
                self.append(word,lm) #will calculate all this automatically?
                
    def subgraph(self,indices,lm):
        """return a copy of the graph acc the indices, first one inclusive, second one exclusive"""
        new = NgramGraph(lm)
        ...
        new.total_unigram_logprob = list(self.total_unigram_logprob[indices[0]:indices[1]])
        return new
    
    def append(self,word,lm):
        """Add a word to the list (right frontier, calculate its prob with its context"""
        #print "appending"
        #print word
        ...
        self.total_unigram_logprob.append(self.total_unigram_logprob[-1] + log(newunigram))
    ...
    def extend(self,graph,lm): #sticking two together
        ...
        for unigram in graph.unigram_graph:
            #print unigram
            self.total_unigram_logprob.append(self.total_unigram_logprob[-1]+log(unigram))    
        self.unigram_graph+=list(graph.unigram_graph) #won't make a difference for unigrams
        for i in range(len(self.word_graph)-len(graph.word_graph),len(self.word_graph)):
            newngram = lm.ngram_prob(self.word_graph[i-self.delta:i+1],lm.order)
            self.prob_graph.append(newngram)
    ...
    def logprob_weighted_by_sequence_length(self,indices=None):
        if indices == None:
            ...
            #pass #TODO
            return float(self.total_logprob[indices[1]]-self.total_logprob[indices[0]]) / float(indices[1]-indices[0])
    
    def logprob_weighted_by_inverse_unigram_logprob(self,indices=None):
        """indices is an int list of length 1 or 2, if 1, just end point, if two, start and end point"""
        if indices == None:
            return float(self.total_logprob[-1]) / float(-1.0 * self.total_unigram_logprob[-1])
        elif (len(indices)==1):
            return float(self.total_logprob[indices[0]]) / float(-1.0 * self.total_unigram_logprob[indices[0]])
        else:
            return float(self.total_logprob[indices[1]]-self.total_logprob[indices[0]]) / float(-1.0 * float(self.total_unigram_logprob[indices[1]] - self.total_unigram_logprob[indices[0]]))
        
class KneserNeySmoothingModel(LanguageModel):
    """The standard implementation of Kneser Ney interpolation. 
    This has specialized decoding, training, loading and saving methods for standard KN."""
    def __init__(self,order=3,discount=None,partial_words=False,
                 train_corpus=None,heldout_corpus=None,second_corpus=None,verbose=True, saved_file=None):
        """
        Keyword arguments:

        train_corpus -- a corpus file or string of words, which if passed to the constructor the model is trained right away (default None)
        heldout_corpus -- a smaller corpus file or string which treats words not in vocab so far as unknown (default None)
        second_corpus -- if present, a corpus file or string which is an additional training source
        order -- the order of the model (3 = trigram, 2 = bigram, etc.) (default 3)
        partial_words -- boolean as to whether this deals with partial words with special probs or not
        ...
        self.train_length = 0 #gets initialized to check the training corpus length
        
        # params of the model which can be loaded or learned
        self.unigrams = []# Just an ordered list of unigrams that are followers (i.e. not <s>)
        self.unigram_counts = defaultdict(int)
        self.bigrams = [] # Essentially just a subset of ngram_denom for the bigram types, but not glued tokens :)
        ...
        self.bigram_history_entropies = defaultdict(float) #For now leaving these out
        ...
        if train_corpus !=None: #this is required to 
            #order the unigrams and bigrams for entropies, can probably discard after entropy cacheing,
            print str(self.train_length), "total words of training data"    
            for ngram,val in sorted(self.unigram_counts.items(), key=itemgetter(1),reverse=True): self.unigrams.append(ngram) #just turn into list?
            self.vocab_size = len(self.unigrams)
            for ngram,val in sorted(self.bigram_counts.items(), key=itemgetter(1),reverse=True): self.bigrams.append(ngram) #just turn into list?
            self.init_entropy_cache(0.40) #initialise max ent and most frequent unigram contexts for bigrams
        
        # if this gets loaded instead of trained, populates everything from pickled db
        if saved_file !=None:
    ...
    def glue_tokens(self,tokens,order):
        """NB this is specific to the way we store these for Kneser Ney smoothing"""
    ...
    def ngrams_interpolated_kneser_ney(self,tokens,order):
        ...
        lower order non zero counts necessary for interpolated Kneser-Ney smoothing,
        ...
        for i in xrange(order-1,l): # tokens should have a prefix of order - 1 <s>s
            ...
            for d in xrange(order,0,-1): #go through all the different 'n's
                if d == 1:
                    ...
                    #print num_key
                    if d == self.order:
                        self.unigram_counts[tokens[i]] += 1 #just takes the last one to get the unigram count
                        self.bigram_counts[' '.join(tokens[i-1:i+1])]+=1 #a raw count of the bigrams, rather than the Kneser-Ney purely type based approach
                    ...
                    if tmp == 0: # if this is the first time we see this ngram
                        self.ngram_non_zero_map[den_key] += 1  #number of types it's been used as a context for
                        if d == 2:
                            self.unigram_contexts[tokens[i-1]].append(tokens[i]) #bit wasteful as essentially building the model again
                            self.bigram_types+=1
                        elif d == 3:
                            self.bigram_contexts[' '.join(tokens[i-(d-1) : i])].append(tokens[i]) # this can be cleared after training as not needed after prob_distribution_over_completions creation 
                            self.trigram_types+=1
                        # we increment the non zero count and implicitly switch to the lower order
    ...
    def train(self,train_corpus,heldout=False):
        """This method creates the model by reading data from a corpus (a file like object open for reading) and trains a model of the given order"""
        if isinstance(train_corpus,str): #if special mode, i.e. string, split on new line character \n
            print "training corpus is a string"
            train_corpus = train_corpus.split("\n") #split string by new line
            is_file = False
        else:
            print "training corpus is a file"
            train_corpus.seek(0) # we reset the corpus reading position to the start
            is_file = True
        ...
        for line in train_corpus:
            if is_file == True: #assuming a REF file here
                text = line.split(",")[-1]
                tokens = self.tokenize_sentence(text,self.order) #if we have a normal tag
                for line in train_corpus: #skip a line
                    break
            else: #just string to parse
                tokens = self.tokenize_sentence(line,self.order) #if we have a normal tag
            if heldout == True:
                oldtokens = list(tokens)
                tokens = []
                for token in oldtokens: #put unknown token in for unknown words, only form of held out est used
                    if (not (token == "<s>" or token == "</s>"))\
                    and (self.ngram_numerator_map.get(self.glue_tokens(token,1)) == None):
                        tokens.append("<unk>")
                        totalunk+=1
                    else:
                        tokens.append(token)
        ...
        self.vocab_size = len(self.unigram_counts.keys()) #always updates after any training
        if heldout == True: 
    ...
    def raw_ngram_prob(self,ngram,discount,order,partialWordFactor=False):
        """The internal implementation of ngram_prob.
           ...
           to use a different discount than the general one we can do it with this function.
        """
        tokens = []
        #filter out unseen 
        partialWord = 1
        ...
        if order >=2 and partialWordFactor==True: #partial words
            if ( ngram[-2][-1]== "-" or\
            (self.ngram_numerator_map.get(self.glue_tokens(ngram[-2],1)) == None\
              and properPrefix(ngram[-2],ngram[-1]) == True)):
                ...
                #raw_input()
                partialWord = 0.00001
        for token in ngram: #put unknown token in for unknown words, only form of held out est used
            if (self.ngram_numerator_map.get(self.glue_tokens(token,1)) == None) \
        ...
        #calculate the unigram prob of the last token as it appears as a numerator
        #if we've never seen it at all, it defacto will have no probability as a numerator
        uni_num = self.ngram_numerator_map.get(self.glue_tokens(ngram[-1],1))
        if uni_num == None: uni_num = 0
        probability = previous_prob = float(uni_num) / float(self.unigram_denominator)
        if probability == 0.0:
            print "0 prob!"
        ...
        for d in xrange(2,order+1):
            ngram_den = self.ngram_denominator_map.get(self.glue_tokens(ngram[-(d):-1],d))
            if ngram_den == None: ngram_den = 0
            #for bigrams this is the number of different continuation types (number of trigram types with these two words)
            if ngram_den != 0: #if this context (bigram, for trigrams) has never been seen, then we can only get unigram est, starts from two, goes up
                ngram_num = self.ngram_numerator_map.get(self.glue_tokens(ngram[-(d):],d)) #this is adding one, use get?
                if ngram_num == None: ngram_num = 0
                if ngram_num != 0:
                    current_prob = (ngram_num - discount) / float(ngram_den)
                else:
                    current_prob = 0.0
                nonzero = self.ngram_non_zero_map.get(self.glue_tokens(ngram[-(d):-1],d))
                if nonzero == None: nonzero = 0
                current_prob += nonzero * discount / ngram_den * previous_prob
                previous_prob = current_prob
                probability = current_prob
            else: #current unseen contexts just give you the unigram back..not ideal.. we can learn <unk> from held out data though..
    ...
    def ngram_prob(self,ngram,order,special=None): 
        ...
        if not special==None:
            return self.raw_ngram_prob_special(ngram,self.discount,order,special)    
        return self.raw_ngram_prob(ngram,self.discount,order,partialWordFactor=self.partial_words)
    ...
    def entropy_continuation(self,contexttokens,order):
        ...
        s = 0
        tokens = []
        for token in contexttokens: #put unknown token in for unknown words, only form of held out est used
            if (self.ngram_numerator_map.get(self.glue_tokens(token,1)) == None) and not token =="<s>": #i.e. never seen at all
                tokens.append("<unk>")
            else:
                tokens.append(token)
    ...
    def entropy_continuation_very_fast(self,contexttokens,order,returneps=False,returnDist=False):
        ...
        if order == 1:
            ...
            return self.max_ent_continuation #i.e. smoothed entropy of anything coming next
        number = self.ngram_non_zero_map.get(self.glue_tokens(contexttokens,order)) #number of types of trigram, should be the same as below
        #print number
        if number == None:
            #print "backing off"
            return self.entropy_continuation_very_fast(contexttokens[1:],order-1,returneps=returneps,returnDist=returnDist) #otherwise go down
        check = 0
        
        if order == 3: positives = self.bigram_contexts.get(' '.join(contexttokens))
        if order == 2: positives = self.unigram_contexts.get(contexttokens[-1])
        ...
        #print "very fast continuation entropy of " + str(contexttokens) + "=" + str(s)
        #print "total mass= " + str(totalMass) +"\n"
        return s
    
    def init_entropy_cache(self,percent):
        """helps with storage of top M n-1 gram contexts' entropies, and the max ent"""
        totalmass= 0
        s = 0
        ...
        assert abs(1.0 - totalmass)<=0.00000000001,"not summing to one" + str(totalmass)
        
        #now estimate the top x% of entropies of trigram and bigram contexts
        #do trigrams
        if self.order == 3:
            self.trigram_history_entropies["<s> <s>"] = self.entropy_continuation_very_fast(["<s>","<s>"],3)
            target = int(float(len(self.bigrams)) * float(percent))
            for i in range(0,target):
                if self.bigrams[i].split()[-1] == "</s>": continue #not useful for this as not a context
                self.trigram_history_entropies[self.bigrams[i]] = self.entropy_continuation_very_fast(self.bigrams[i].split(),3)
            #print len(self.trigram_history_entropies)
        #now do bigrams
        self.bigram_history_entropies["<s>"] = self.entropy_continuation_very_fast(["<s>"],2)
        target = int(float(len(self.unigrams)) * float((percent))) #in reality, percent should be greater for bigrams
        for i in range(0,target):
            if self.unigrams[i] == "</s>": continue #not useful for this
            self.bigram_history_entropies[self.unigrams[i]] = self.entropy_continuation_very_fast([self.unigrams[i]],2)
            
    def sum_information_gain_very_fast(self,tokens,order=None):
        """computes entropy of the context tokens and actual probability of each ngram to give the total uncertainty reduction"""
        if order == None: order = self.order
        delta = order - 1
        return sum([log(-self.logprob_weighted_by_sequence_length(tokens[i-delta:i+1])/self.entropy_continuation_very_fast(tokens[i-delta:i],order)) for i in range(delta,len(tokens))])
        
    def KL_divergence_continuation(self,contexttokens1,contexttokens2):
        """Can make this general, just tricky because of unique glue_tokens method.
            NB it's assymetric.
            Assuming normal distributions for lower order(s)- speeds things up a lot
            Only allow this full version for POS model as it's quick, need something quicker otherwise
        """
        totalMass1 = 0
        totalMass2 = 0
        KL = 0
        ...
        for key in self.unigrams: #LOOK AT ALL NGRAMS /unigrams
            ...
            if p1 == 0:
                newKL = 0
                #raw_input("KL prob = 0!! " + str(contexttokens1) + str(target))
            elif p2 == 0:
                print "INFINITE KL DIVERGENCE!" #todo should we still check they're proper prob dists?
                return infinity
            else:
                newKL = log(p1/p2) * p1
            KL += newKL
        assert abs(1.0-totalMass1)<=0.00000000001 and abs(1.0-totalMass2)<=0.00000000001,\
        "NOT SUMMING TO 1. total mass 1 = " + str(totalMass1) + " total mass 2 = " \
    ...
    def KL_divergence_continuation_fast(self,contexttokens1,contexttokens2):
        """Faster version of KL_divergence_continuation which approximates unseen counts by smoothing.
        """
        #tic = time.clock()
        totalMass1 = 0
        totalMass2 = 0
        ...
        #if contexttokens1[-1] == contexttokens2[-1]: bigram = True #they have the same bigram context, will save time
        check = 0
        #vocab = [] #if not seen before, try it
        zeroCount1 = [] #the probs of context 2 + target that have zero counts for context tokens 1
        zeroCount2 = [] #the probs of context 1 + target  that have zero counts for context tokens 2
        bothZero = 0
        for key in self.unigrams: #LOOK AT ALL NGRAMS /unigrams

            test = str(key)
            #assuming normal prob_distribution_over_completions over unigrams only (i.e. leaving out unseen bigrams)
            zero1 = False
            #zero2 = False
            if self.ngram_numerator_map.get(self.glue_tokens([contexttokens1[-1],test],2)) == None:
                zero1 = True
            if self.ngram_numerator_map.get(self.glue_tokens([contexttokens2[-1],test],2)) == None:
                #zero2 = True
                if zero1 == True:
                    bothZero+=1
                else:
    ...
    def KL_divergence_very_fast_matrix(self, p, q):
        ...
        We need to store an array for each bigram history linked to its target in v
        We can store entropy too doing this in an easy way.
        The problem is the initial run time for calcuating these. Given we've got sparse vectors
    ...
    def store_top_N_matrices(self,n):
        ...
        #self.matrices = OrderedDict(bsr) #simple list of top bigram histories indexed to a sparse matrix, their epsilon and their entropy
        for bigram in self.bigrams.keys():    
            self.ngram_denominator_map[bigram]
        i = 0
        for bigram,val in sorted(bigram.items(), key=itemgetter(1),reverse=True): #most fertile bigrams
            self.cache[bigram] = self.entropy_continuation_fast(bigram.split(),returnEps=True,returnDist=True) #returns the epsilon and the sparse matrix
            i+=1
            if i == n: break
    
    def prob_distribution_over_completions(self, prefix):
        prefix = prefix[:-1] # assuming getting rid of the - prefix tries only way to do this
        probdist = defaultdict(float)
        Z = 0  #normalising constant
        missing = 0
        for key in self.unigrams.keys():
            if not prefix in key == None:
                missing+=1; continue #TODO what's the best way of smoothing this?
        ...
        print prefix + ":___\n"
        totalMass = 0
        for key,val in sorted(probdist.items(), key=itemgetter(1),reverse=True): #can get entropy on line for this?
            newprob = (1/Z) * probdist[key]
            probdist[key] = newprob
            totalMass+=newprob
        #print totalMass #should always sum to 1
        assert (abs(1.0-totalMass)<=0.00000000001), "NOT SUMMING TO 1. total mass  = " + str(totalMass) + " " + prefix
        return probdist
    
    def raw_prob_completion(self,prefix,completion): #gives raw prob of a completion given its prefix, well, smoothed because of sparse data
        ...
        if prefix[-1] == "-":
            prefix = prefix[:-1]
        return float(self.prefixMap.get(self.glue_tokens(prefix,1)+self.glue_tokens(completion,1))) / self.prefixDict[prefix] #take MLE
    ...
    def prob_fluent(self,ngram,order):
        ...
        for unigram in self.unigrams.keys():
            ...
            if completionprob == 0:
                ...
                prob = self.raw_prob_completion(ngram[-1],unigram) * self.ngram_prob(test,order)
            #prob += epsilon # just to smooth, we give everything the possibility of being one maximum entropy value greater than 0
            if not prob == 0.0:
                count+=1
            Z+=prob
            if prob>highest:
    ...
    def prob_fluent_simple(self,ngram,order):
        return self.ngram_prob(ngram,order) * 0.075 #simply using the average drop in fluency from corpus estimate
...
#         self.train_length = attrs['train_length'] #gets initialized to check the training corpus length
#         
#         self.bigram_types = attrs['bigram_types']  
#         self.trigram_types = attrs['trigram_types'] 
#         self.vocab_size = attrs['vocab_size'] 
#         self.unigram_denominator = attrs['unigram_den'] # for unigrams we don't have contexts
#         
...
#         return self
    ...
    def load(self, index_filename):
        ...
        #self.bigram_history_entropies = attributes['bigram_history_entropies'] # TODO for now not doing the entropy cacheing
        #self.trigram_history_entropies = attributes['trigram_history_entropies']
    ...
    def save(self,index_filename):
        ...
        cPickle.dump({'smoothing' : 'kneser-ney',\
            'order' : self.order, 'partial_words': self.partial_words, 'discount' : self.discount, 'verbose' : self.verbose,\
            'bigram_types' : self.bigram_types, 'trigram_types' : self.trigram_types, 'vocab_size' : self.vocab_size, 'unigram_denominator' : self.unigram_denominator,\
            'ngram_numerator_map' : self.ngram_numerator_map, 'ngram_denominator_map' : self.ngram_denominator_map, 'ngram_non_zero_map' : self.ngram_non_zero_map,\
            'unigram_contexts': self.unigram_contexts, 'bigram_contexts': self.bigram_contexts,\
            'unigram_counts' : self.unigram_counts, 'bigram_counts': self.bigram_counts,\
...
#         unigram_contexts_db = os.path.abspath(index_filename) + '.unigram_contexts.db'
#         bigram_contexts_db = os.path.abspath(index_filename) + '.bigram_contexts.db'
#         unigram_counts_db = os.path.abspath(index_filename) + '.unigram_counts.db'
#         bigram_counts_db = os.path.abspath(index_filename) + '.bigram_counts.db'
#         bigram_history_entropies_db = os.path.abspath(index_filename) + '.bigram_history_entropies.db'
#         trigram_history_entropies_db = os.path.abspath(index_filename) + '.trigram_history_entropies.db'
...
#                      'order' : self.order, 'partial_words': self.partial_words, 'discount' : self.discount, 'verbose' : self.verbose,\
#                      'bigram_types' : self.bigram_types, 'trigram_types' : self.trigram_types, 'vocab_size' : self.vocab_size, 'unigram_den' : self.unigram_denominator,
#                     'num_db' : num_db, 'den_db' : den_db, 'nz_db' : nz_db, 'unigram_contexts_db': unigram_contexts_db, 'bigram_contexts_db': bigram_contexts_db,\
#                     'unigram_counts_db' : unigram_counts_db, 'bigram_counts_db': bigram_counts_db, 'bigram_history_entropies_db' : bigram_history_entropies_db,\
#                     'trigram_histories_db' : trigram_history_entropies_db, 'unigrams_db': unigrams, 'bigrams_db': bigrams},f)
#         flush_and_close(f)