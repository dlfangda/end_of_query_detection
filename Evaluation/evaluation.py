...
from train_models import get_train

from evaluation_utils import *
from get_probability_distributions_LR import *

sys.path.append("../../")
#from hmm_decoder.hmm import rnn_hmm
#from hmm_decoder.generate_trp_file import gen_trans_file

print "pandas version, needs to be 0.19+", pd.__version__
...
def get_test(speaker,eps_path,return_individual_fold=False):
    
    
    train = pd.DataFrame() if not return_individual_fold else [] #list if folds
    pathlist = lsdir(eps_path+'r'+str(int(speaker))+'/', '.pkl')
...
def train_MMT(mode, subset, LG_path, speak):
    ...
    #open pkl distributions array and convert to dataframe
    #for now the simplest method to get the data in
    fname = LG_path+'/r'+str(speak)+'/prob_dist_r'+str(speak)+'_'+mode+'_'+subset+'.pkl'
    with open(fname,'rb') as pkl_array:
        y_np = pickle.load(pkl_array)

    px2 = y_np.reshape((-1,4))
    y = pd.DataFrame({'mtp':px2[:,0],'speech':px2[:,1],'trp':px2[:,2],'episode':px2[:,3]})
    
    x = y
    x = x[['episode','mtp','speech','trp']]
...
def fold_with_padding(features, fold, seq_len, scaler):
...
#
    ...
    for e in range(num_epochs):
        print "epoch", e+1
        scores = []
        for fold in train: # should only one epoch
            y, X = fold_with_padding(features, fold, seq_len, scaler)
            ...
            print y
            print y.shape
            predictions = model.predict_classes(X, batch_size=1, verbose=2)
            ...
            p_r_f_tags = precision_recall_fscore_support(new_y,predictions, average='weighted')
            test_result = p_r_f_tags[2]
            #print p_r_f_tags
            tag_summary =  classification_report(new_y,predictions,labels=
        ...
        if score > best:
            ...
            continue
        if e - best_epoch > 3: break # no imporvement
...
wml + wml_trigram'
    ...
    for speaker in range(start,end):
        ...
        test = get_test(speaker,episodes_path,return_individual_fold=model_name=='lstm')
        for mode in modes:
            ...
            modpath = models_path+model_name + "_allatonce_1_layer_16_nodes"
           # if not os.path.isdir(modpath):
            #    os.makedirs(modpath)
            ...
            if model_name == "lstm":
                ...
                #    scaler = joblib.load(fd)

                n_classes = 3
                print 'testing model...'
                test_LSTM(test, features, n_features, n_classes, scaler, prepath+'/'+str(speaker)+'_'+mode+r'.h5')                
...
from Logistic regression model for speaker',speaker, ' at\n', LG_path
            ...
            #joblib.dump(scaler,spath) #save the scaler
...
#requirements: output files at ./../../model_name/r(speaker)/ _ | distributions/
#args:
#model MMT | Logistic_Regression #LSTM can be built in here
#
def eval_MMT_LR_final(model_name='MMT', \
                  prepath='./../../',\
                  start=2, end=8,\
                  modes=['acoustic','lm','acousticlm'],\
                  subset='test'):
    ...
    for mode in modes:
        ...
        if model == 'MMT':
            ep_dict = load_final_out_dict(prepath,start,end,mode,subset)
        elif model == 'Logistic_Regression':
            ep_dict = load_dict(prepath, start, end, mode, subset)            

        #10ms window label accuracy
        acc_dict, prec_recall_dict = get_accuracy_precision_recall(model,\
                                                                   prepath,\
                                                                   start,\
                                                                   end, subset)
        print_accuracy_class_report(acc_dict, prec_recall_dict,\
                                start, end, mode, subset)
        
    #EOT accuracy

    for mode in modes:

        time_precision_dict = get_t_precision_dict(start, end, mode, subset)

        for speaker in range(start, end):
            print mode, '{:.2f}'.format(time_precision_dict[speaker][time_key])

        print '\n'            
        avgMMT, avgLR = avg_EOTacc(time_precision_dict, start, end)
        if model == 'MMT':
            print 'Average: ',avgMMT
        elif model == 'Logistic_Regression':
            print 'Average: ',avgLR
        print '\n\n'
        ...
        '''Plot'''
        #for mode in modes:
            #dist_dict_LR = get_temporal_diff_distribution(2, 8, 'tp', mode, subset)
            #dist_dict_MMT = get_temporal_diff_distribution(2, 8, 'ts', mode, subset)
            #for sp in range(start, end):
                #plot_temporal_difference_distribution(speaker, dist_dict_LR, dist_dict_MMT, 'Temporal difference from gold standard (0.0) for speaker '+str(speaker)+' with '+mode+' features', bins_tp=60, bins_ts=17, for_all=False, plegend=True)

    return

def eval_MMT_incremental(model_name='MMT', \
                  prepath='./../../',\
                  start=2, end=8,\
                  modes=['acoustic','lm','acousticlm'],\
                  subset='test'):
    ...
    for mode in modes:
        print 'measurement correction time'
        print 'percentage of corrected EOT per threshold'
        corr_dict = real_corrections(prepath+model_name+'/',\
                                     start, end, mode, subset)
        print print_corrections_per_threshold(corr_dict[0],\
                                                  corr_dict[1],\
                                                  corr_dict[2],\
                                                  mode, start, end)
        '''Plot function'''
        #uncomment to plot correction times frequency, maybe set xmax to 1.0 for a better view 
        #plot_correction_delay(corr_dict[0], start, end, xmin=0.0, xmax=15.0, bin_tl=100, plegend=True)

        print 'Edit overhead (100*(#operations/sequence_length)-1)'
        eo_dict = compute_edit_overhead(prepath+model_name+'/', start, end, mode, subset)
        '''Mean and sd of Edit overhead per speaker'''
        print_eo(eo_dict, start, end)
        '''Plot function'''
        #uncomment to plot eo distribution over all speakers
        #first argument: speaker if argument for_all=False
        #plot_eo_distribution(2, eo_dict, 'Histogram of edit overhead for all speakers', plegend=True, bins_tp=50, for_all=True)

    
    return
if __name__ == "__main__":
    episodes_path = "./../../pickledEpisodes/"
    models_path = "./../../"
    model_name = 'lstm' #or LogisticRegression, MMT
    ##########################################################
    ...
    if model_name == 'lstm':
        ...
        #Get probability distributions from Logistic Regression
        #only once
        #filepath to the directory of the probability distributions
        #./../../Logistic_Regression/r(speaker)/distributions/

        #for speaker in range(start, end):
        #    for mode in modes:
        #        prob_dist_dt_loop(speaker, mode, subset)

        
        #Evaluate final system output (label sequence)
        eval_MMT_LR_final(model_name, \
                      models_path,\
                      start=2, end=8,\
                      modes=['acoustic','lm','acousticlm'],\
                      subset='test')

        #Evaluate incremental system output
        #correction time:
        eval_MMT_incremental(model_name='MMT', \
                  prepath='./../../',\
                  start=2, end=8,\
                  modes=['acoustic','lm','acousticlm'],\
                  subset='test')
